...
def payment(request):
    ...
    if doc is not None:
        context = {"MID": "fDlkIy64148311435937", "TXN_AMOUNT": str(doc.price), "ORDER_ID": str(doc.order_id),
                   "CUST_ID": str(request.user.pk).zfill(64), "CHANNEL_ID": "WEB", "INDUSTRY_TYPE_ID": "Retail",
                   "WEBSITE": "WEBSTAGING",
                   "CALLBACK_URL": "http://127.0.0.1/test_payments.php"}
        CHECKSUMHASH = generate_checksum(context, "Z1IzyJW2BvkU8pWX")
        print(context)
        print(CHECKSUMHASH)
        return render(request, "ground/payment.html",
                      {'CHECKSUMHASH': CHECKSUMHASH, "MID": "fDlkIy64148311435937", "TXN_AMOUNT": str(doc.price),
